/*
 * $Id: FactoryFinder.java,v 1.37.4.4 2013/08/12 16:22:57 edburns Exp $
 */
...
public final class FactoryFinder {
    ...
    // Can be called via reflection, so the "no usages" warning should be ignored.
    // Test cases may need to call it this way.
    ...
    private static final class FactoryManagerCache {
        ...
        private FactoryManager getApplicationFactoryManager(ClassLoader cl, String factoryName) {
            FactoryManager result = null;
            FactoryManagerCacheKey curKey;
            FactoryManager curValue;
            
            for (Map.Entry<FactoryManagerCacheKey, FactoryManager> cur : applicationMap.entrySet()) {
                curKey = cur.getKey();
                curValue = cur.getValue();
                if (curValue.hasFactory(factoryName)) {
                    result = curValue;
                    break;
                }
            }
    ...
    private static final class FactoryManager {
        ...
        private boolean hasFactory(String factoryName) {
            boolean result = false;
            
            Object factoryOrList = factories.get(factoryName);
            if (null != factoryOrList) {
                if (factoryOrList instanceof List) {
                    result = 0 < ((List)factoryOrList).size();
                } else {
                    result = true;
                }
            }
        ...
        public Object getFactory(ClassLoader cl, String factoryName) {
            ...
            try {
                ...
                if (factory == null) {
                    ...
                    FactoryManager manager =
                            FACTORIES_CACHE.getApplicationFactoryManager(cl, factoryName);
                    if (null != manager) {
                        factory = manager.getFactoryWithoutFallback(cl, factoryName);
                    }
        ...
        public Object getFactoryWithoutFallback(ClassLoader cl, String factoryName) {
