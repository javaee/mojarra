...
public class RequestParameterMapFactory {
	...
	public RequestParameterMapFactory(ServletRequest request) {
		String contentType = request.getContentType();

		if ((contentType != null) && contentType.toLowerCase().startsWith(MULTIPART_CONTENT_TYPE_PREFIX)) {
			RequestParameterMapMultiPartImpl requestParameterMapMultiPartImpl = new RequestParameterMapMultiPartImpl(request);
			requestParameterMap = requestParameterMapMultiPartImpl;
			requestParameterValuesMap = new RequestParameterValuesMapMultiPartImpl(request, requestParameterMapMultiPartImpl);
		}
		else {
