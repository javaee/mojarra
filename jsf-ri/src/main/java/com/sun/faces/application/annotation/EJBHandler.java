 ...
 * annotations.
 */
class EJBHandler implements RuntimeAnnotationHandler {
    ...
    private void applyToField(FacesContext facesContext, Field field, EJB ejb, Object object) {
        if (ejb.lookup() != null && !"".equals(ejb.lookup().trim())) {
            try {
                InitialContext context = new InitialContext();
                Object ejbObject = context.lookup(ejb.lookup());
                boolean fieldAccessible = true;
                if (!field.isAccessible()) {
                    field.setAccessible(true);
                    fieldAccessible = false;
                }
                field.set(object, ejbObject);
                if (!fieldAccessible) {
            ...
            try {
                InitialContext context = new InitialContext();
                Object ejbObject = context.lookup("java:comp/env/" + ejb.name());
                boolean fieldAccessible = true;
                if (!field.isAccessible()) {
                    field.setAccessible(true);
                    fieldAccessible = false;
            ...
            try {
                InitialContext context = new InitialContext();
                Object ejbObject = context.lookup("java:module/" + field.getType().getSimpleName());
                boolean fieldAccessible = true;
                if (!field.isAccessible()) {
                    field.setAccessible(true);
                    fieldAccessible = false;
