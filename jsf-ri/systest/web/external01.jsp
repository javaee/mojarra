...
<%
    ...
    if (request.getAttribute(key) != null) {
        out.println(
              "/external01.jsp FAILED - can not remove ServletContext attribute");
        return;
    }
    facesContext.getExternalContext().getRequestMap().remove(key);
    if (facesContext.getExternalContext().getRequestMap().get(key) != null) {
        out.println(
              "/external01.jsp FAILED - can not remove application scope attribute");
        return;
    }
    ...
    actual =
          (String) facesContext.getExternalContext().getRequestMap().get(key);
    if (!value1.equals(actual)) {
        out.println("/external01.jsp FAILED - ServletContext attribute set to '"
                    +
                    value1
