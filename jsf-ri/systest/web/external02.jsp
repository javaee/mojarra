...
<%
    ...
    if (session.getAttribute(key) != null) {
        out.println(
              "/external02.jsp FAILED - can not remove ServletContext attribute");
        return;
    }
    facesContext.getExternalContext().getSessionMap().remove(key);
    if (facesContext.getExternalContext().getSessionMap().get(key) != null) {
        out.println(
              "/external02.jsp FAILED - can not remove application scope attribute");
        return;
    }
    ...
    actual =
          (String) facesContext.getExternalContext().getSessionMap().get(key);
    if (!value1.equals(actual)) {
        out.println("/external02.jsp FAILED - ServletContext attribute set to '"
                    +
                    value1
