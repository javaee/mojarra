...
<%
    ...
    if (application.getAttribute(key) != null) {
        out.println(
              "/external03.jsp FAILED - can not remove ServletContext attribute");
        return;
    }
    ...
    {
        out.println(
              "/external03.jsp FAILED - can not remove application scope attribute");
        return;
    }
    ...
    if (!value1.equals(actual)) {
        out.println("/external03.jsp FAILED - ServletContext attribute set to '"
                    +
                    value1
                    + "' but Servlet API returned '"
                    + actual
                    + "'");
        return;
    }
    actual = (String) facesContext.getExternalContext().getApplicationMap()
          .get(key);
    if (!value1.equals(actual)) {
        out.println("/external03.jsp FAILED - ServletContext attribute set to '"
                    +
                    value1
                    + "' but Faces API returned '"
                    + actual
                    + "'");
    }
