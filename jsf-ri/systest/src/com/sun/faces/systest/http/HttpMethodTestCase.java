...
import junit.framework.TestSuite;
 ...
 public class HttpMethodTestCase extends HtmlUnitFacesTestCase {
    ...
    public void testPositive() throws Exception {
        int [] rc = new int[1];
        String exp = "(?s).*Begin\\s*test\\s*jsp include without verbatim\\s*interweaving\\s*works\\s*well!!\\s*End\\s*test\\s*jsp include without verbatim.*";
        // Ensure the GET request works as expected
        assertTrue(issueHttpRequest("GET", rc).matches(exp));
        ...
        assertTrue(issueHttpRequest("GETBOGUSALLOWED", rc).matches(exp));
        assertEquals(HttpURLConnection.HTTP_OK, rc[0]);
     ...
     public void testNegative() throws Exception {
        ...
        assertFalse("Bogus HTTP method was accepted by server.  Fail.",
                issueHttpRequest("GET22", rc).matches(exp));
        assertFalse("Bogus HTTP method returned HTTP_OK status.  Fail.", HttpURLConnection.HTTP_OK == rc[0]);
     }
    
    private String issueHttpRequest(String methodName, int [] rc) throws Exception {

        URL url = getURL("/faces/interweaving01.jsp");
        Socket s = new Socket(url.getHost(), url.getPort());
        BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(s.getOutputStream()));
        String requestLine = methodName + " /jsf-systest/faces/interweaving01.jsp HTTP/1.1\r\n";
